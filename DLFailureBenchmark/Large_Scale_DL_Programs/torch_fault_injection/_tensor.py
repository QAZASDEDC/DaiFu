...
class Tensor(torch._C._TensorBase):
    def to(self, *args, **kwargs):
        if torch.PLANNED_FAULT_TYPE == 'GPU':
            torch.PLANNED_FAULT_TYPE = None
            torch.logger.info("Fault Injected To: " + str(torch.INJECT_LOCATION))
            raise RuntimeError("Injected Fault: to")
