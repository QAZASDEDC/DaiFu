...
def fault_injection_init_for_outter_multiprocess_start():
    multiprocessing.current_process().name = "MainProcess-" + str(eval(os.environ['LOCAL_RANK']) + 2)

    global injection_location_file
    injection_location_file = Path.cwd()/'daifu_workspace' / (multiprocessing.current_process().name + '_injection_location.pkl')

    global injection_location_file_for_epoch 
    injection_location_file_for_epoch = Path.cwd()/'daifu_workspace' / (multiprocessing.current_process().name + '_injection_location_for_epoch.pkl')

PLANNED_FAULT_TYPE = None
...
(Path.cwd()/'daifu_workspace').mkdir(exist_ok=True)

injection_location_file = Path.cwd()/'daifu_workspace' / (multiprocessing.current_process().name + '_injection_location.pkl')
injection_location_path = Path.cwd()/'daifu_workspace'
injection_location_files = injection_location_path.glob('*_injection_location.pkl')
for ilf_file in injection_location_files:
    with ilf_file.open('rb') as f:
        INJECT_LOCATION = pickle.load(f)
        break
...
def fault_injection_scheduler_and_recorder(freq, i, epoch, nprocs, is_end = False):
    global PLANNED_FAULT_TYPE
    global INJECT_LOCATION
    if nprocs == 1 or str(i%nprocs+2) in multiprocessing.current_process().name:
        if INJECT_LOCATION is None and not is_end:
            if i % freq == 0:
                INJECT_LOCATION = (i, epoch)
                PLANNED_FAULT_TYPE = 'GPU'#np.random.choice(['Network', 'GPU']) if i !=0 else 'GPU'

                with injection_location_file.open('wb') as f:
                    pickle.dump(INJECT_LOCATION, f)
        elif INJECT_LOCATION == (i, epoch) and is_end:
            global IS_RESUME_SUCCESS
            if IS_RESUME_SUCCESS:
                logger.info(str(INJECT_LOCATION) + " Fault Recovered")
            else:
                logger.info(str(INJECT_LOCATION) + " Fault Not Properly Recovered, Please Use the Default Checkpoint Recovery Time for this Injection")
                IS_RESUME_SUCCESS = True
            injection_location_file.unlink()
            INJECT_LOCATION = None
    elif INJECT_LOCATION == (i, epoch) and is_end:
        INJECT_LOCATION = None

IS_PLANNED_NETWORK_FAULT = False
IS_PLANNED_DISK_FAULT = False
INJECT_LOCATION_FOR_EPOCH = None

injection_location_file_for_epoch = Path.cwd()/'daifu_workspace' / (multiprocessing.current_process().name + '_injection_location_for_epoch.pkl')
injection_location_path_for_epoch = Path.cwd()/'daifu_workspace'
injection_location_files_for_epoch = injection_location_path_for_epoch.glob('*_injection_location_for_epoch.pkl')
for ilf_file in injection_location_files_for_epoch:
    with ilf_file.open('rb') as f:
        INJECT_LOCATION_FOR_EPOCH = pickle.load(f)
        break

def fault_injection_scheduler_and_recorder_for_epoch(epoch, nprocs, is_end = False):
    global IS_PLANNED_NETWORK_FAULT
    global IS_PLANNED_DISK_FAULT
    global INJECT_LOCATION_FOR_EPOCH
    if nprocs == 1 or str(epoch%nprocs+2) in multiprocessing.current_process().name:
        if INJECT_LOCATION_FOR_EPOCH is None and not is_end:
            IS_PLANNED_NETWORK_FAULT = True
            IS_PLANNED_DISK_FAULT = True
            INJECT_LOCATION_FOR_EPOCH = epoch

            with injection_location_file_for_epoch.open('wb') as f:
                pickle.dump(INJECT_LOCATION_FOR_EPOCH, f)
        elif INJECT_LOCATION_FOR_EPOCH == epoch and is_end:
            global IS_RESUME_SUCCESS
            if IS_RESUME_SUCCESS:
                logger.info(str(INJECT_LOCATION_FOR_EPOCH) + " Fault Recovered")
            else:
                logger.info(str(INJECT_LOCATION_FOR_EPOCH) + " Fault Not Properly Recovered, Please Use the Default Checkpoint Recovery Time for this Injection")
                IS_RESUME_SUCCESS = True
            injection_location_file_for_epoch.unlink()
            INJECT_LOCATION_FOR_EPOCH = None