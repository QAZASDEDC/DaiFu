...
class CFIterator:
	def __init__(
			...
			persist=True,
			cf_manager=None):
		if (not isinstance(dataloader, Iterable)) or callable(dataloader):
			raise ValueError("Dataloader of type {} is not iterable nor callable".format(type(dataloader)))

		self._dataloader = dataloader
		if callable(self._dataloader):
			self._iterator = self._dataloader
		else:
		...
		self._max_steps_per_epoch = len(self._dataloader)#int(math.ceil(self._size / self._batch_size))
		...
		self._arch = arch
		self._cache_file = "checkfreq_workspace/.cache_" + self._arch + "_" + str(self._batch_size)
		self._steps_since_chk = 0
		self.t_start = 0
		self._dynamic = dynamic
		# default MANUAL determine frequency
		self._chk_mode = CFMode.MANUAL
		self.current_length = len(self._dataloader)
	...
	def __iter__(self):
		if callable(self._iterator):
			...
			self._iterator = iter(self._dataloader)
			if self.current_length < len(self._dataloader):
				steps_to_skip = len(self._dataloader) - self.current_length
				#for i in range(steps_to_skip):
				#	next(self._iterator)
				self._iterator._sampler_iter = islice(self._iterator._sampler_iter, steps_to_skip, None)
		return self
	...
	def load_state_dict(self, chk_map):
		...
		self._total_steps = self._epoch * self._max_steps_per_epoch + self._steps_this_epoch
		self.current_length = len(self._dataloader) - self._steps_this_epoch
