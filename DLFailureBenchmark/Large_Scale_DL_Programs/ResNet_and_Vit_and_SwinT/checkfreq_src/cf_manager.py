...
class CFManager:
		...
		def save(
			...
			persist=False):
				...
				use_thread = True# otherwise it crashes because RuntimeError: Default process group has not been initialized, please make sure to call init_process_group.
				if use_thread:
		...
		def save_cpu(
			...
			use_thread=True):
				...
				if use_thread:
					...
					fn = globals()["Process"].get_context('fork')
				print("Function is {}".format(fn))
				if not is_epoch:
					keywords = { \
						'iter_chk':self.available_chk_iters, \
