...
def plot_img(self, image):
    daifu_store = {}
    for daifu_dataname in daifu.TRANSFORM_REGISTRY['plot_img']['dataname_list'
        ]:
    ...
    try:
        daifu_return_tag, daifu_return_item = plot_img_daifu_cell_1()
    except Exception:
        while True:
            try:
                daifu.RP_MANAGER.repair()
                daifu_return_tag, daifu_return_item = plot_img_daifu_rest_1()
                break
            except Exception:
                continue
